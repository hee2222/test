const info = [
  {
    title: '전략/조직설계',
    color: ['#F3F0ED', '#CF471C', '#CF471C30'],
    desc: '전략 직무의 특징은 전략 직무를 시작하는 분들을 위한 기본기부터 SK에서 가장 Hot한 전략적 Agenda를 접목하기 위한 Skill을 획득할 수 있습니다.',
    slide: [
      [
        [
          [
            'OJT와 함께 Entry 인력의 기본기 강화에 학습을 활용',
            '전략 Foundation에 해당하는 영역별 실무지식, 방법론 학습',
          ],
          ['신입/전입 등 전략 실무 경험 3년 이하 구성원 대상'],
        ],
        [
          [
            'SK그룹 경영 Agenda를 사별 Biz./조직 특성, 산업/경쟁 현황 등을 고려하여 적용 및 Practice 확보',
            '일 경험을 통한 체득이 어려운 미래 직무 관점 upskilling 중심',
          ],
          [
            '전략 5~15년차 구성원 中 주제별 선발',
            '연간 Agenda를 반영하여 Hot skill 중심 topic 선정',
          ],
        ],
      ],
      [
        [
          '전략 입문자를 위한 기본기 다지기',
          [
            '직무별 주요 개념',
            ['[Strategy Essential] 경영전략 기본 개념과 방법론 이해', 1],
            [
              '[조직 Self-Design Essential] 현업 리더가 알아야 할 조직 설계 기본',
              2,
            ],
            [
              '[Financial Story] 되고 싶은 나를 구현하기 위한 파트너십 구축 이해',
              3,
            ],
            [
              '[Scenario Planning Essential] 시나리오 플래닝 방법론 및 사례 이해',
              4,
            ],
          ],
        ],
        [
          '전략 실무자가 알아야 할 경영 성과관리',
          [
            '실무활용',
            ['전략 실행과 성과관리', [1]],
            ['전략 Leader를 위한 재무회계관리'],
            ['하이닉스 End to End R&D 조직'],
            ['경영 환경 변화에 따른 조직설계 Challenge', [2]],
            ['Employee Experience와 조직 설계'],
            ['Customer Centric 조직으로의 진화'],
          ],
          [
            '미래 역량_사업성장기반 구축',
            ['해외 거점 기반 Biz. 전개', [1]],
            ['[Workshop] Global Market Penetration'],
            ['[Workshop] Cross Border M&A Intensive'],
          ],
          [
            '미래 역량_Mgmt. System 재정비',
            ['기업가치 핵심인자 기반 경영시스템 변화:Overview'],
            ['기업가치 핵심인자 기반 경영성과관리: Case Study'],
            ['[Workshop] 기업가치 핵심인자 기반 경영성과관리', [3]],
            ['[Workshop] Risk Sensing & Response Planning', [4]],
          ],
        ],
      ],
      [
        [
          [
            '객관식 Test - 총 20문항 40분',
            '전문가 인터뷰 -총 10-15문항, 1시간 내외 (대면 / 비대면 선택 가능)',
            '80점 이상 Pass, 재응시 1회 가능 (연 내)',
          ],
          [
            [
              'Strategy Essential',
              'Strategy Essengtial은 전략 수립을 위한 내/외부 경영 환경 분석 방법론 개념과 활용',
              [
                '전략 101: 어서 와, 전략은 처음이지?',
                '전략 수립의 시작, 경영환경분석',
                'All about Portfolio Strategy',
                '[과제] 경영환경분석 실습 Report',
              ],
            ],
            [
              '조직 Self Design Essential',
              '리더가 알아야 할 조직 설계의 기본 개념과 방법론',
              [
                '조직 Self-Design과 Deep Change',
                '조직 설계 101',
                '조직 설계 기초 Framework',
                '리더를 위한 조직 Self-Design 실전',
                'SK 조직 설계 Re:think',
                '[조직 설계] 역량 Self-Check',
              ],
            ],
          ],
        ],
        [
          [
            '과정 주제와 연계한 PJT 수행 / 발표  문제해결 및 적용 방식을 기술 ',
            '80점 이상 Pass, 재응시 1회 가능 (연 내) (사별 정책에 따라 재 응시가 가능하거나,  과정 수강과 별개로 진행할 수 있습니다.)',
          ],
          [
            [
              '재무적/비재무적  핵심인자 도출 및 관리',
              '기업가치에 영향을 미치는 재무적/비재무적 핵심인자의 도출 및 관리 방안',
              ['기업가치 핵심인자관리'],
            ],
            [
              '해외 기업 인수합병 의사결정',
              '해외 기업 인수합병 의사결정',
              ['Cross Border M&A'],
            ],
            [
              '해외 진출 전략 수립',
              '해외 진출 전략 수립',
              ['Global Market Penetration'],
            ],
            [
              'Macro 기반 잠재 Risk 진단 및 경영대응 계획 수립',
              'Macro 기반 잠재 Risk 진단 및 경영대응 계획 수립',
              ['Risk Sensing & Response Planning'],
            ],
          ],
        ],
      ],
    ],
  },
  {
    title: '재무회계',
    color: ['#EDF3F2', '#0AA39C', '#0AA39C30'],
    desc: '재무회계 직무의 특징은 단계별 직무수행에 따른 재무 주요 Issue에 대해 학습할 수 있으며, 재무회계 뿐 아니라 Investment 관점에서도 폭넓은 재무회계를 이해할 수 있는 과정입니다.',
    slide: [
      [
        [
          [
            '재무회계 직무 수행을 위한 필수 지식과 경험이 필요한 시기',
            '각 영역의 업무 기초 개념, 방법론 등 기본기 제공 및 평가/인증',
          ],
          ['신입/전입 등 재무 실무경험 3년 이하 구성원 대상'],
        ],
        [
          [
            '‘일을 통한 성장’ 과 ‘전문성 향상’ 을 위한  도약이 가능하도록 지원',
            '현장 Issue와 실무 Agenda 중심의 직무  역량 향상 및 평가/인증',
          ],
          [
            '재무/회계 경력 3년차 이상의 실무자',
            '제도 변화 및 현업적용 방안 등 실무 관점 Guide 제공',
            '단기 미래 Agenda를 반영하여 Hot skill 중심 topic 선정',
          ],
        ],
        [
          [
            'Financial Story 실행 지원을 위한 투자/자본시장 전문가 육성',
            'Capstone Project 시행을 통해 문제 해결역량에 대한 평가/인증',
          ],
          [
            '미래 재무 리더 후보 군으로 팀장/PL 보임 전 참여',
            'M&A, 투자 의사결정 등 Biz. 전략과 연계한 특수 목적 직무역량 대상',
          ],
        ],
      ],
      [
        [
          '구성원 모두를 위한 재무/회계의 기초',
          [
            '직무별 주요 개념',
            ['[Fun-Fun한 회계]'],
            ['[궁금해요, 내부회계관리제도]'],
            ['[기업가치, 무엇이고 어떻게 평가하는가?]'],
            ['[세무회계 실무 첫걸음]'],
            ['[재무관리 101]'],
            ['[처음 만나는 원가/관리회계]'],
            ['[하나씩 쉽게, 계정과목 별 회계처리]'],
            ['세무회계의 정석(上)  (下)'],
            ['전사원이 함께하는 회계원리(上)  (下)'],
            ['한 달 만에 끝내는 원가관리회계(上)  (下)'],
            ['한 달 만에 끝내는 재무제표분석(上)  (下)'],
            ['Financial Story 개념과 사례'],
            ['Market Watch #1. 코로나 임팩트 : 금융시장 전문가의 시각'],
            ['Market Watch #2. Global Top Tier PE Firms'],
            ['Market Watch #3. IB 및 증권사 리서치센터에서 본 Key Trends'],
            ['내부회계관리제도'],
            ['내부회계관리제도 중요성의 이해'],
            ['7분 만에 읽는 <내러티브 & 넘버스>'],
            ['이동훈 투자센터장의 ’금융기관 둘러보기’'],
            ['황이석 교수의 Financial Acumen'],
          ],
          [
            'Data 개념 및 M/L 기초',
            ['CDS I_데이터 처리와 시각화', 1],
            ['CDS II_머신러닝 모델링'],
          ],
        ],
        [
          '재무실무자를 위한 영역별 심화 과정',
          [
            '실무 활용',
            ['M&A 전략의 성공적인 의사결정을 위한 재무회계 실무'],
            ['2. 분할 전략의 성공적인 의사결정을 위한 재무회계 실무'],
            ['3. 효과적인 M&A를 위한 재무실사(FDD)'],
            ['자금관리와 금융 실무'],
            ['SKC의 IR 개선 사례'],
            ['사례중심의 세무회계실무(上) (下)'],
            ['사례중심의 재무회계실무(上) (下)'],
            ['자금관리와 금융 기초'],
            ['한 달 만에 끝내는 경영분석실무(上) (下)'],
            ['한 달 만에 끝내는 기업가치평가(上) (下)'],
            ['한 달 만에 끝내는 사업타당성분석(上) (下)'],
            ['전문가와 함께 하는 <내러티브 & 넘버스> 북 콘서트 - 1부 & 2부'],
          ],
          [
            'Data 기반 재무 의사결정',
            ['[Workshop] Data 기반 의사결정 지원', [1]],
          ],
          ['글로벌 ESG 공시', ['[Workshop] 기업가치 제고를 위한 ESG']],
          [
            'Global 재무 관리',
            ['[Workshop] 해외 자산관리 실무'],
            ['[Workshop] 해외 투자 Valuation'],
          ],
          ['효율적 투자관리', ['[Workshop] EXIT 전략 수립']],
        ],
        [
          '재무전문가를 위한 ...',
          [
            '투자/Deal 재무 전문 역량 확보',
            ['[Off-line, 선발] 자본시장 전문가'],
          ],
          [
            '투자 관리 전문 역량 확보',
            ['[Off-line, 선발] Portfolio 관리 전문가'],
          ],
        ],
      ],
      [
        [
          [
            '객관식 Test  - 총 20문항 40분',
            '전문가 인터뷰  - 총 10~15문항, 1시간 내외 (대면/비대면 선택 가능)',
            '80점 이상 Pass, 재응시 1회 가능 (연 내)',
          ],
          [
            [
              '재무회계 Essential',
              '재무회계의 기본 용어정의 및 프로세스에  대한 이해',
              [
                'Fun-Fun한 회계',
                '궁금해요, 내부회계관리제도',
                '기업가치, 무엇이고 어떻게 평가하는가?',
                '재무관리 101',
                '한 달 만에 끝내는 재무제표분석(上)  (下)',
                '하나씩 쉽게, 계정과목 별 회계처리',
              ],
            ],
            [
              '원가관리회계 Essential',
              '원가의 기본 개념  및 원가별 기초 계산과 적용에 대한 기본  지식 보유 검증',
              [
                ' 처음 만나는 원가/관리회계',
                '한 달 만에 끝내는 원가관리회계(上)  (下)',
              ],
            ],
            [
              '자금관리 Essential',
              '자금관리의 기초 개념 및 자금관리 기법 습득에 대한 기초 이해 및 활용 검증',
              ['자금 관리와 금융 기초'],
            ],
          ],
        ],
        [
          [
            '과정 주제와 연계한 PJT 수행 / 발표  문제해결 및 적용 방식을 기술 ',
            '80점 이상 Pass, 재응시 1회 가능 (연 내) (사별 정책에 따라 재 응시가 가능하거나,  과정 수강과 별개로 진행할 수 있습니다.)',
          ],
          [
            [
              'ESG 기준 재무 적용',
              'ISSB 회계기준 적용한 지표 작성 및 ESG 적용 시 주요 Issue에 대한 해결방안 도출',
              ['(기업가치 제고를 위한) ESG Workshop'],
            ],
            [
              'AI를 활용한 재무/회계 운영 실무',
              'ChatGPT를 활용한 재무정보 결과값 도출',
              ['Data 기반 의사결정 지원 Workshop'],
            ],
            [
              '기업가치평가(Valuation) 실무',
              'Cross-border M&A 시 Valuation 추정 및 Issue에  대한 해결방안 수립',
              ['Cross-border M&A Valuation 실무 Workshop'],
            ],
            [
              '해외자산 운영/관리 실무',
              'Cross-border M&A 시 Valuation 추정 및 Issue에  대한 해결방안 수립',
              ['Global 자산관리 실무 Workshop'],
            ],
            [
              'EXIT 전략 수립 실무',
              'PMI이후 보유한 해외자사관리의 재무 Issue별  문제 해결방안 수립',
              ['EXIT 전략 수립 실무 Workshop'],
            ],
          ],
        ],
        [
          [
            '과정 주제와 연계한 PJT 수행 / 발표  문제해결 및 적용 방식을 기술',
            '해당 과정 이수 시 인증 가능하나, 20% 우수 평가자에 대해서는 그룹 SKOUGHT에 등록할 예정입니다.',
          ],
          [
            [
              'M&A Deal 전문 실무',
              'M&A Deal 기획/운영/관리 전반에 대한 PJT Leading 역량 보유 여부',
              ['자본시장전문가 과정'],
            ],
          ],
        ],
      ],
    ],
  },
  {
    title: '마케팅',
    color: ['#F1F3ED', '#8E783F', '#8E783F30'],
    desc: '마케팅 직무의 특성은 시장/트렌드 분석, 고객 경험 Design 및 효과적인 Comm. 전략 등  고객/시장 관련 다양한 제반 지식과 Framework 을 체계적으로 학습함으로써 현업 문제 해결을 위한 실무 역량을 제고합니다.',
    slide: [
      [
        [
          [
            '마케팅 유관 직무 수행 시 리더/구성원 모두 반드시 알아야 하는 기본적 지식 습득',
            '마케팅 믹스 등 마케팅 전략 수립과 관련한 기초개념, Framework, 기본기  중심',
          ],
          ['신입/전입 등 마케팅 유관 업무 경험 3년 이하 구성원 대상'],
        ],
        [
          [
            '실무 현장에서 필요로 하는 전문 지식, Know-how 및 직무의 미래 혁신/변화 아젠다 관련 학습',
            '4개의 Hot-Skill(B2C / B2B  마케팅 각 2개씩) 을 중심으로 커리큘럼 구성',
          ],

          ['마케팅 유관 업무 3년차 이상 실무자'],
        ],
        [
          [
            '데이터 마케팅 프로젝트를 리드할 수 있는 그룹 Talent 수준의 전문가 육성 과정 ',
          ],
          [
            '고객/시장 데이터 분석 관련 업무 5년차 이상 구성원 or  그에  상응하는 DT 마케팅 관련 background 보유자',
          ],
        ],
      ],
      [
        [
          '마케팅 입문자를 위한 기본기 다지기',
          [
            '마케팅 Essential',
            ['키워드로 배우는 마케팅 101'],
            ['B2B2C 마케팅 101', 1],
            ['사례로 배우는 상품기획 101', 2],
            ['Global 마케팅 사례분석'],
            ['마케팅 Framework & Tool'],
          ],
          [
            'Data 마케팅 101',
            ['Data 마케팅 Transformation', 3],
            ['Data 마케팅 Concept & Case', 4],
            ['퍼포먼스 마케팅'],
            ['Brand New Wave : AI 마케팅', 5],
            ['디지털 채널 경쟁력 분석'],
            ['SK의 고객 Data 기반 Deep Change'],
            ['마스터카드의 DT 성공 전략 Case'],
          ],
          ['B2B 실무 101', ['B2B 실무 101', 6]],
          [
            'Data 개념 및 M/L 기초',
            ['CDS I_데이터 처리와 시각화', 7],
            ['CDS II_머신러닝 모델링'],
          ],
        ],
        [
          '마케팅 실무 전문성 제고를 위한 Hot Skill',
          [
            'B2B 마케팅 Pipeline 관리',
            ['[Workshop] B2B 마케팅 Pipeline 관리', [1, 3]],
          ],
          [
            'DT 기반 고객 Engagement',
            ['[Workshop] DT 기반 고객 Engagement', [2, 4]],
          ],
          [
            'Data Driven Marketing',
            ['[Workshop] Data driven Marketing', [5, 7]],
          ],
          ['Strategic 제안 영업', ['[Workshop] Strategic 제안 영업', [2, 6]]],
        ],
        [
          'DT 마케팅 Expert',
          [
            'DT 마케팅 Expert',
            ['공통 집합 교육 (2주간 @KAIST)', [2, 4]],
            ['Project Based Learning (약 3달간)', [5, 7]],
          ],
        ],
      ],
      [
        [
          [
            '객관식 Test - 총 20문항 40분',
            '80점 이상 Pass',
            '재응시 1회 가능 (연 내)',
          ],
          [
            [
              '마케팅 Essential',
              '마케팅 개념 및 적용기초',
              [
                '키워드로 배우는 마케팅 101',
                'B2B2C 마케팅 101',
                '사례로 배우는 상품기획 101',
                'Global 마케팅 사례분석',
                '마케팅 Framework & Tool',
              ],
            ],
            [
              'Data 마케팅 101',
              '고객/시장 데이터 분석 관련 기초적 개념 및 Case',
              [
                'Data 마케팅 Transformation',
                'Data 마케팅 Concept & Case',
                '퍼포먼스 마케팅',
                'Brand New Wave : AI 마케팅',
                '디지털 채널 경쟁력 분석 ',
                'SK의 고객 Data 기반 Deep Change',
                '마스터카드의 DT 성공 전략 Case',
              ],
            ],
            [
              'B2B 실무 101',
              '(B2C 와 구별되는) B2B 시장 및 고객의 특성 및 실무 체계',
              ['B2B 실무 101'],
            ],
          ],
        ],
        [
          [
            '과정 주제와 연계한 PJT 수행 / 발표  문제해결 및 적용 방식 기술',
            '80점 이상 Pass',
            '재응시 1회 가능 ( 총 2회 / 연)',
          ],
          [
            [
              'Analytic 아젠다 도출 및 마케팅 전략 수립',
              '현업 고객/시장 Data 분석 및 Analytic Agenda 도출 역량 검증',
              ['Data driven Marketing'],
            ],
            [
              '고객 경험(CX) 디자인 설계',
              'DT 환경에서의 고객 Engagement 강화를 위한   MKT 전략 수립 역량 보유 여부 검증',
              ['DT 기반 고객 Engagement'],
            ],
            [
              'Lead Lifecycle 분석 기반 고객 발굴/관리',
              '현업에서 담당중인 B2B 마케팅 & Sales 파이프라인 강화 전략 수립 역량 검증',
              ['B2B 마케팅 Pipeline 관리'],
            ],
            [
              '(PT) 제품/서비스  Value Proposition',
              '실무 마케팅 환경에서의 효과적인  Value Proposition 기반 수주 관리 역량 보유 여부 검증',
              ['Strategic 제안 영업'],
            ],
          ],
        ],
        [
          [
            '2주간 공통 집합 교육',
            '약 3달 여간의 전문가(KAIST 경영학과 교수진)과의 1 on 1 코칭 기반 Project Based Learning 방식',
            'SK 그룹 DT 자문 임원단 및 KAIST 경영대 교수진의 심사를 받아 최종 인증',
            '과정 수료자 중 평가 상위 20% 내외의 학습자 선발,  그룹 Expert로 등재',
          ],
          [
            [
              '마케팅 Expert',
              '고객/시장 데이터 분석 및 데이터간 상호 작용 (모델링) 기반 새로운 관점의 View/Insight  도출',
              ['마케팅 전문가 과정'],
            ],
          ],
        ],
      ],
    ],
  },
  {
    title: 'HR',
    color: ['#F3EDED', '#C20D4E', '#C20D4E30'],
    desc: 'HR 과정의 특징은 입문자를 위한 기초과정과 더불어 외부 교육과정 또는 일 경험을 통해 쉽게 습득할 수 없는 Skill 함양을 통해 미래 Agenda에 선제적인 대응력 강화,  통합적 전문성과 전략적 시각 확보에 초점을 맞추고 있습니다.',
    slide: [
      [
        [
          [
            'OJT와 함께 Entry 인력의 기본기 강화에 학습을 활용',
            'HR Foundation에 해당하는영역별 실무지식, 방법론 학습',
          ],
          ['신입/전입 등 HR 실무경험 3년 이하 구성원 대상'],
        ],
        [
          [
            'HR 세부 직무별 전문성에 대한 ‘일을 통한 육성’이 중요한 시기',
            '일 경험을 통한 체득이 어려운 미래 직무 관점 upskilling 중심',
            '단기(2~3년내) 대내외 미래 변화관리 Agenda와 연계, 운영',
          ],
          [
            'HR 5~15년차 구성원 中 주제별 선발',
            '연간 Agenda를 반영하여 Hot skill 중심 Topic 선정',
          ],
        ],
        [
          [
            '리더 보임 전 마지막 관문으로 통합적 HR 전문성과 전략적 시각 확보 목적',
            'In-depth study, 현업 문제 해결, 의사결정 능력에 대한 평가/인증',
          ],

          [
            '미래 HR 리더 후보 군으로 팀장/PL 보임 전 참여',
            'HRM, HRD, ER 등 Track  구분없이 통합하여 선발 및 참여',
          ],
        ],
      ],
      [
        [
          'HR입문자를 위한 기본기 다지기',
          [
            '직무별 주요 개념',
            ['[인력 확보 Essential] 인력계획 및 채용의 이해', 1],
            ['[직무 관리 Essential] 직무 및 직무 활용의 이해', 2],
            ['[성과관리 Essential] 성과관리의 개념 및 프로세스 이해', 3],
            ['[보상 Essential] 보상 제도의 이해'],
            ['[Global HR Essential] 주재원 및 Global Staff 이해', 5],
            ['[ER Essential] 인력 운영 규제의 이해', 6],
            ['기업가치기반 보상: 주식기반 보상 이해'],
          ],
          ['Talent Mgmt. Essential', ['면접위원 교육 과정(Essential)']],
          [
            'Data 개념 및 M/L 기초',
            ['CDS I_데이터 처리와 시각화', 7],
            ['CDS II_머신러닝 모델링'],
          ],
        ],
        [
          'HR 실무 전문성 제고를 위한 Core Skill',
          [
            '실무 활용',
            ['AI를 활용한 역량 기반 비대면 채용 이해', [1]],
            ['BM혁신을 위한 조직역량 관리 (Skill based HR)', [2]],
            ['변화의 시대, 달라진 성과관리와 보상', [3]],
            ['기업가치기반 보상: 주식기반 보상 운영 실무'],
            ['[Workshop] 기업가치기반 보상(주식기반 보상을 중심)'],
          ],
          [
            'Data Driven HR',
            ['[HR Analytics] ① 사례로 알아보는 HR Analytics'],
            ['[HR Analytics] ②Foundation of HR Analytics'],
            ['[HR Analytics] ③Practice of HR Analytics'],
          ],
          ['HR Global Intelligence', ['[Workshop] HR DD/PMI']],
        ],
        [
          'HR Foresight & 통합적 HR Expert',
          ['통합 HR 전문가', ['[Off-line, 선발] Strategic HR Expert', [1]]],
          ['통합 ER 전문가', ['[Off-line, 선발] ER Exper']],
          [
            'HR Insight',
            ['[특강] 전략적 HR 실천 (HR 변화와 담당자 역할 전환)'],
            ['[특강] 일하는 방식을 혁신하라'],
            ['2020 SK HR Conference'],
            ['2021 HR Perspective Seminar (1차/2차)'],
            ['2022 SK HR Conference : HR New Approach'],
            ['[이천포럼 2021] 뜨거운 감자, 제도와 공정'],
            ['HR을 위한 memo, 어제의 먼 미래 현실이 되다'],
          ],
        ],
      ],
      [
        [
          [
            '전문가 인터뷰  - 총 10~15문항, 1시간 내외 (대면/비대면 선택 가능)',
            '80점 이상 Pass',
            '재응시 1회 가능 (연 내)',
          ],
          [
            [
              'HR Essential',
              '인력 확보, 직무관리, 성과관리, 보상, Global HR, ER  영역 전반 운영을 위한 기초 지식 보유 여부',
              [
                '[인력 확보 Essential] 인력계획 및 채용의 이해',
                '[직무 관리 Essential] 직무 및 직무 활용의 이해',
                '[성과관리 Essential] 성과관리의 개념 및 프로세스 이해',
                '[보상 Essential] 보상 제도의 이해',
                '[Global HR Essential] 주재원 및 Global Staff 이해',
                '[ER Essential] 인력 운영 규제의 이해',
              ],
            ],
          ],
        ],
        [
          [
            'Test Only, Test + 과제 제출 중 각 Skill  검증에 적합한 방식으로 출제',
            '과정 주제와 연계한 PJT 수행 / 발표  문제해결 및 적용 방식을 기술',
            '80점 이상 Pass, 재응시 1회 가능 (연 내)',
          ],
          [
            [
              '주식기반보상 설계/운영',
              '주식보상 유형별 개념 및 특징, 세무,Legal 이슈 관련 지식 보유 여부, 주식보상 설계 방법론 기반 최적 설계방안 제시',
              [
                '기업가치기반 보상(주식기반 보상을 중심으로) : 기업가치기반 보상 이해',
                '기업가치기반 보상 (주식기반보상을 중심으로):  Workshop',
                '기업가치기반 보상(주식기반 보상을 중심으로): 주식기반 보상 운영 실무',
              ],
            ],
            [
              'HR DD/PMI (예정)',
              'Cross Border M&A 상 Pre –deal DD, PMI 과정상  HR Risk 파악 및 대응방안 수립',
              [
                '[M&A] Ⅰ. M&A Overview',
                'Cross Border M&A HR Risk 점검 (예정)',
                'Cross Border M&A HR Risk 점검: Intensive Workshop (예정)',
              ],
            ],
            [
              '미국 노사관계 대응전략 수립 (예정)',
              '미국 연방법원 노동법 및 주별 규정에 따라 발생가능한 이슈 파악 및 대응방안 수립',
              ['미국 노사관계 실무 과정 (예정)'],
            ],
            [
              '미국 주식보상설계/운영 (예정)',
              '미국 연방법원 노동법 및 주별 규정에 따라 발생가능한 이슈 파악 및 대응방안 수립',
              [
                '기업가치기반 보상(주식기반 보상을 중심으로): 기업가치기반 보상 이해',
                '미국 조직 보상 설계/운영 Workshop (예정)',
              ],
            ],
          ],
        ],
        [
          [
            '각 과정 주제와 연계한 PJT 수행 / 발표로 진행되며,',
            '과정별 Leader Committee를 운영,  현업 리더가 직접 Coaching/평가',
            '과정 수료자 중 평가 상위 20% 내외의 학습자 선발,  그룹 Expert로 등재',
          ],
          [
            [
              'HR Expert',
              '그룹 전략방향과 Align한 HR Issue에 대한 Insight 보유 정도 검증',
              ['Strategic HR Expert 과정'],
            ],
            [
              'ER Expert',
              '노사관계를 둘러싼 거시적 환경 및 멤버사 개별 이슈를 종합적으로 고려한 노사관계 방향성 및 ER 이슈 대응방안 도출에 있어 Insight 보유 정도 검증',
              ['ER Expert 과정'],
            ],
          ],
        ],
      ],
    ],
  },
  {
    title: '구매/SCM',
    color: ['#F2F2F2', '#575757', '#57575730'],
    desc: '구매/SCM 직무의 특징은 전사적 관점의 SCM 최적화 및 Industry Value Chain 관점에서 관리 Skill을 획득할 수 있습니다.',
    slide: [
      [
        [
          [
            'OJT와 함께 Entry 인력의  기본기 강화에 학습을 활용',
            '전략 Foundation에 해당하는 영역별 실무지식, 방법론 학습',
          ],
          ['신입/전입 등 전략 실무 경험 3년 이하 구성원 대상'],
        ],
        [
          [
            '일 경험을 통한 체득이 어려운  미래 직무 관점 upskilling 중심',
            '단기(2~3년내) 대내외 미래 변화관리 Agenda와 연계, 운영',
          ],
          [
            '구매/SCM 5~15년차 구성원 中 주제별 선발',
            '연간 Agenda를 반영하여  Hot skill 중심 topic 선정',
          ],
        ],
      ],
      [
        [
          '구매와 SCM의 기본기 다지기',
          [
            '직무별 주요 개념',
            ['구매의 본질'],
            ['구매 업무 프로세스 이해 및 활용'],
            ['구매 품목의 이해 및 관리'],
            ['구매 계약/구매 관련 법규 이해'],
            ['구매인을 위한 재무/회계/물류'],
            ['구매 위험 관리'],
            ['디지털 구매 혁신'],
            ['기업의 사회적 가치와 구매'],
            ['공정거래법 – 하도급법'],
            ['구매와 타 부서와의 협력과 연계'],
            ['성공적인 구매  협상'],
            ['Procurement 혁신 – 구매 프로세스의 재구성'],
            ['글로벌 공급망 Dynamics 대응'],
          ],
        ],
        [
          '구매 실무자를 위한 핵심 개념 & Framework',
          [
            '구매 원가',
            ['기업의 성공을 위한 전략적 구매'],
            ['전략적 공급자 관리'],
            ['[Workshop] 최적 구매원가 관리'],
          ],
          ['구매 Risk', ['SCM 최적화 전략 수립'], ['글로벌 공급망 Risk 관리']],
        ],
      ],
      [
        [
          [
            '객관식 Test  - 총 20문항 40분',
            '전문가 인터뷰  - 총 10~15문항, 1시간 내외 (대면/비대면 선택 가능)',
            '80점 이상 Pass, 재응시 1회 가능 (연 내)',
          ],
          [
            [
              '구매 Essentials',
              'Strategy Essential은 전략 수립을 위한 내·외부 경영 환경 분석 방법론 개념과 활용',
              [
                '나는 진정한 구매인인가? 구매의 본질',
                '구매의 큰 그림 보기, 구매 업무 프로세스 이해 및 활용',
                '나는 무엇을 구매하고 있나? 구매 품목의 이해 및 관리',
                '구매의 중요한 역할! 구매 원가 관리',
                '모르면 큰 코 다치는, 구매 계약/구매 관련 법규 이해',
                '알 수록 돈 버는, 구매인을 위한 재무/회계/물류',
                '방심은 금물! 구매 위험 관리',
                '디지털로 스마트해지는 구매, 디지털 구매 혁신',
                '다음 단계를 생각하는 기업의 사회적 가치와 구매',
                '시뮬레이션을 통해 알아보는 실전 구매전략',
              ],
            ],
          ],
        ],
        [
          [
            '과정 주제와 연계한 PJT 수행 / 발표  문제해결 및 적용 방식을 기술',
            '80점 이상 Pass, 재응시 1회 가능 (연 내) (사별 정책에 따라 재 응시가 가능하거나,  과정 수강과 별개로 진행할 수 있습니다.)',
          ],
          [
            [
              '표준원가 기획',
              '구매원가 관련 방법론/개념 확인 및 가상기업 Case 기반 제조원가 및 표준원가 도출',
              ['최적 구매원가 관리'],
            ],
            [
              '재고의 이익 Impact 산정',
              '재고가 기업의 이익에 미치는 Impact 산정',
              ['SCM 최적화 전략 수립'],
            ],
            [
              '공급망 Risk 진단 및 대응 전략 수립',
              'Supply Chain 단계별 Risk 평가 및 대응 방안 수립',
              ['글로벌 공급망 Risk 관리'],
            ],
          ],
        ],
      ],
    ],
  },
  {
    title: 'IP',
    color: ['#EDF1F3', '#076DCC', '#076DCC30'],
    desc: 'IP 직무 과정의 특징은 IP 담당자뿐만 아니라 지식재산을 창출하는 사업직군의 구성원들이 IP에 대한 중요성과 대응 방안에 대해 쉽게 이해할 수 있도록 하며, Global IP업무에도 실질적으로 대응 가능하도록 법률 전문가 수준의 학습을 제공하고 있습니다.',
    slide: [
      [
        [
          [
            'OJT와 함께 Entry 인력의  기본기 강화에 학습을 활용',
            'IP Foundation에 해당하는 영역별 실무지식, 방법론 학습',
          ],
          ['신입/전입 등 IP 실무 경험 3년 이하 구성원 대상'],
        ],
        [
          [
            'IP 과정 별 전문성에 대한  ‘일을 통한 육성’이 중요한 시기',
            '일 경험을 통한 체득이 어려운  미래 직무 관점 upskilling 중심',
            '단기(2~3년내) 대내외 미래 변화관리 Agenda와 연계, 운영',
          ],
          [
            'IP 5~15년차 구성원 中  주제별 선발',
            '연간 Agenda를 반영하여  Hot skill 중심 topic 선정',
          ],
        ],
        [
          [
            'Biz. 전략과 연계한 IP 전문성과  전략적 시각 확보 목적',
            'In-depth study, 현업 문제 해결, 의사결정 능력에 대한 평가/인증',
          ],
          [
            '미래 후보 군으로 팀장/PL 보임 전 참여',
            'HRM, HRD, ER 등 Track  구분없이 통합하여 선발 및 참여',
          ],
        ],
      ],
      [
        [
          '기술경쟁력 확보의 첫걸음! 지식재산 올인원',
          [
            'IP Literacy',
            ['[IP Mind-Set] ① IP, 한 번에 All Kill: 지식재산권 101'],
            ['[IP Mind-Set] ②연구개발과 지식재산'],
            ['[IP Mind-Set] ③ 직무발명'],
            ['[IP Mind-Set] ④ AI, 특허를 만나다! MAKE AI PATENT'],
            ['[IP Mind-Set] ⑤ IP War! News로 이해하는 특허분쟁'],
            ['[IP Mind-Set] ⑥ 분쟁대응 문서관리'],
            ['성공적인 사업화 Starting point : IP 계약'],
          ],
          [
            'Open Source',
            ['[오픈소스] ① 미래를 밝히는 오픈소스 선형적 발전사'],
            ['[오픈소스] ② Software, Copyright & License'],
            ['[오픈소스] ③ 오픈소스 라이선스'],
          ],
        ],
        [
          '실무자를 위한 특허관리 및 라이센싱/소송',
          [
            '특허관리 실무',
            ['[특허관리] ① 한국 출원제도와 법'],
            ['[특허관리] ② 한국 특허출원 실무'],
            ['[특허관리] ③ 특허활용과 매입'],
            ['[특허관리] ④ M&A와 IP Transaction'],
            ['[Webinar] ML/AI를 활용한 특허 정보 분석과 실무 적용 전략 특강'],
            ['[Workshop] 전략적 미국 특허출원 실무(MPEP)'],
          ],
          [
            '특허 Licensing / 소송 실무',
            ['한국 특허분쟁의 이해와 전략'],
            ['미국 특허분쟁 Key Points'],
            ['특허협상과 라이선스 실무'],
            ['BM 혁신을 위한 특허경영'],
          ],
          [
            'Global 특허제도 & 법',
            ['기술 보호 관점에서 바라보는 미국 특허제도와 법'],
            ['사업관점에서 알아보는 중국 특허제도&법'],
          ],
        ],
        [
          'IP Expert',
          ['통합 IP 전문가', ['[Off-line, 선발] 특허 분쟁 전문가 (PLP)']],
        ],
      ],
      [
        [
          [
            '객관식 Test  - 총 20문항 40분',
            '80점 이상 Pass, 재응시 1회 가능 (연 내)',
          ],
          [
            [
              'IP Mind-Set Essential',
              '인증은 IP의 기초 개념 및 방법론, Process 등 IP의 기본 개념정의 및 기초 활용방안에 대한 역량 보유 여부',
              [
                '[IP Mind-Set] ① IP, 한 번에 All Kill: 지식재산권 101',
                '[IP Mind-Set] ②연구개발과 지식재산',
                '[IP Mind-Set] ③ 직무발명',
                '[IP Mind-Set] ④ AI, 특허를 만나다! MAKE AI PATENT',
                '[IP Mind-Set] ⑤ IP War! News로 이해하는 특허분쟁',
                '[IP Mind-Set] ⑥ 분쟁대응 문서관리',
              ],
            ],
          ],
        ],
        [
          ['객관식 Test', '미국 Patent Bar 시험의 모의 형태', '총 20문항 90분'],
          [
            [
              '미국 특허출원 및  심사 대응',
              '미국 특허 출원 상 발생(가능)한 이슈 및 이와 유관한 조항의 정확한 파악, 적용 시 기한 등을 고려한 신속한 대응가능 여부',
              ['전략적 미국 특허출원 실무(MPEP'],
            ],
          ],
        ],
        [
          [
            '약 3달 여간의 전문가(KAIST 지식재산대학원 )과의 1 on 1 코칭 기반 Project Based Learning 방식',
            'SK 그룹 IP 현업 리더 및 KAIST 지식재산대학원 교수의 심사를 통한 최종 인증',
            '과정 수료자 중 평가 상위 20% 내외의 학습자 선발,  그룹 Expert로 등재',
          ],
          [
            [
              '미국 특허분쟁 전문가 (PLP)',
              '미국 특허 소송 건에 대해 원고/피고로 나누어 소송 Process를 모의로 진행',
              ['미국 특허분쟁 전문가 (PLP)'],
            ],
          ],
        ],
      ],
    ],
  },
  {
    title: '법무',
    color: ['#EFEDF3', '#5347DD', '#5347DD30'],
    desc: '법무 직무의 특징은 기업법무에 대한 이해도를 제고하고 전문성을 강화할 수 있는데 도움을 제공 한다는 것입니다.',
    slide: [
      [
        [
          [
            '현재 Level 1 과정은 전무한 상황이므로 신규 콘텐츠 중심의 업로드 예정',
            '‘Global 지역별 계약서 분석과 작성‘,  ‘기업 법무의 A to Z’ Category 신설 및 법률 기초 개념과 용어 이해',
          ],
          [
            '신입/전입 등 법무 실무 경험 3년 이하 구성원',
            '非 법무 직군 일반 구성원',
          ],
        ],
        [
          [
            '전문성을 旣 보유한 법무 조직 리더/ 구성원들의 Upskilling 관점',
            'Negotiation Partner로서의 Legal Part 역할 강화 등 그룹 Agenda 측면에서 접근',
            'Level 1의 Category를 준용',
          ],
          ['법무 5~15년차 리더/구성원'],
        ],
      ],
      [
        [
          '법무 기본기 다지기',
          [
            '기업 법무 기초',
            ['News를 통해 보는 법무 상식'],
            ['미국 바이든 대통령의  중국 군산복합체 투자금지 행정명령 서명'],
            ['[법률 상식] 외국환거래법 위반 시 제재와 대응방안'],
          ],
          ['Global 법무 기본', ['북미 사업 계약서 분석 기본']],
        ],
        [
          '실무 전문성 제고를 위한 Hot Skill',
          [
            '공정거래법 주요 내용 및 개정상 시사점',
            ['[공정거래법] (1) 공정거래법 이해 및 정책동향'],
            ['[공정거래법] (2) 부당지원, 사익편취'],
            ['[공정거래법] (3) 불공정거래와 부당표시광고'],
            ['[공정거래법] (4) 경제력 집중 억제'],
            ['공정거래법] (5) 기업결합의 제한'],
            ['[공정거래법](6)시장지배적지위 남용 행위'],
            ['[공정거래법] (7) 부당한 공동행위'],
            ['[공정거래법] (8) 하도급법'],
          ],
          [
            '회사법 개정으로 인한 주요 Risk 및 대응 방안',
            ['[회사법] (1) 주주총회 운영실무'],
            ['[회사법] (2) 이사회 운영실무'],
            ['[회사법] (3) 상장 및 공시 이해'],
            ['[회사법] (4) 상장회사 특례'],
          ],
          [
            '최근 법률 쟁점과 우리의 대응',
            ['ESG의 법적 쟁점 및 기업의 대응 전략'],
            ['BREXIT 현실화: RECAP(회고 및 전망)'],
            ['ESG 관련  분쟁사례  소개 및 대응방안'],
            ['[최근 법률 쟁점] 영업 비밀 보호와 침해 예방'],
            [
              '[최근 법률 쟁점] 러시아의 우크라이나 침공에 대응한 미국 등 국제사회의 對러 제재',
            ],
            [
              '[최근 법률 쟁점] 기업지배구조 및 주주총회 운영 관련 최근 쟁점 및 시사점',
            ],
            [
              '[최근 법률 쟁점] 플랫폼경쟁법: 구글쇼핑 사건과 자기우대(self-preferencing) 규제',
            ],
            ['[최근 법률 쟁점] 형사소송법 개정에 따른 수사 및 재판 실무 변화'],
            ['[최근 법률 쟁점] 압수수색에 대해 알아 두어야 할 것들'],
            ['[최근 법률 쟁점] 집회, 시위 등 금지 가처분'],
            ['[최근 법률 쟁점] 중대재해처벌법의 주요 쟁점 및 대응방안'],
            [
              '[최근 법률 쟁점] 개정 상법/공정거래법의 주요내용, 실무적 유의사항 및 관련 규제 동향',
            ],
          ],
          ['Global 지역별 계약서 분석과 작성', ['북미 사업 계약서 작성 실무']],
        ],
      ],
      [
        [
          [
            '객관식 Test - 총 20문항 40분',
            '80점 이상 Pass, 재응시 1회 가능 (연 내)',
          ],
          [
            [
              'Global 투자 계약서 분석 (예정)',
              'Global 투자 계약서의 검토 필요성을 이해하고, 주요 Issue별 관련 조항들을 분석',
              ['Global 투자 계약서 분석'],
            ],
          ],
        ],
        [
          [
            '객관식 Test  - 총 20문항 (40분)',
            '80점 이상 Pass, 재응시 1회 가능 (연 내) (사별 정책에 따라 재 응시가 가능하거나,  과정 수강과 별개로 진행할 수 있습니다.)',
          ],
          [
            [
              '공정거래법 Advanced',
              '공정거래법 관련 최근 규제 동향, 주요 이슈 이해 여부와 관련 Risk 에  대응하기 위한 업무 수행 역량의 보유 여부',
              [
                '[공정거래법] (1) 공정거래법 이해 및 정책동향',
                '[공정거래법] (2) 부당지원, 사익편취',
                '[공정거래법] (3) 불공정거래와 부당표시광고',
                '[공정거래법] (4) 경제력 집중 억제',
                '[공정거래법] (5) 기업결합의 제한',
                '[공정거래법] (6) 시장지배적지위 남용행위',
                '[공정거래법] (7) 부당한 공동행위',
                '[공정거래법] (8) 하도급법',
              ],
            ],
          ],
          [
            '회사법 Advanced',
            '회사법 법령의 쟁점을 알고 관련 Risk에 대응할 수 있는지 여부',
            [
              '[회사법] (1) 주주총회 운영실무',
              '[회사법] (2) 이사회 운영실무',
              '[회사법] (3) 상장 및 공시 이해',
              '[회사법] (4) 상장회사 특례',
            ],
          ],
          [
            '북미 투자 계약서 Advanced (예정)',
            '북미 투자 계약서의 법적 분석 및 해석, 초안 작성 및 검토, 위험 관리 및 컴플라이언스 역량의 보유 여부',
            ['Global 투자 계약서 분석'],
          ],
        ],
      ],
    ],
  },
  // {
  //   desc: '',
  // },
];

export default info;
